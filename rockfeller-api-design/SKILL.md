...
3. Decide whether write needs `Idempotency-Key`, concurrency control, audit, provider adapter, queue or webhook verification. Use `@rockdev/telemetry` for request ID, logs, traces, metrics and errors; never recreate those layers manually.
...
- For a NestJS service, integrate `@rockdev/telemetry` instead of a custom request-ID middleware, logger, OpenTelemetry setup, redaction or exception filter. Its current default error body needs compatibility review before use with this API envelope.
